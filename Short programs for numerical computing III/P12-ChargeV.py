...
nn  = [0]*4
sty = [0]*4                       # end-indexes, styles of plots
col = [""]*4                                                # colors of plots

m = 1e0                                                    # mass of particle
q = -1.e0
Q = 1.e0                                           # in units of e
x0 = 1e0
y0 = 0e0                                         # initial position
vx0 = 0e0
vy0 = 4.5e0                                     # initial velocity
tmax = 20e0                                                       # time span
ht = 0.1e0                                                   # time step size

nt = int(tmax/ht + 0.5) + 1                            # number of time steps
tt = [0]*(3*nt+1)
Et = [0]*(3*nt+1)                        # plotting arrays
xt = [0]*(nt+1)
yt = [0]*(nt+1)
...
it = 1
x = x0
vx = vx0                                             # initial values
y = y0
vy = vy0

(fx, fy, Epot) = Forces(m,x,y,vx,vy)    # initial forces and potential energy
ax = fx/m
ay = fy/m                                   # initial acceleration
Ekin = 0.5e0 * m * (vx**2 + vy**2)
xt[it] = x
yt[it] = y                                   # store for plotting
tt[it] = tt[nt+it] = t
tt[2*nt+it] = t
Et[it] = Ekin
Et[nt+it] = Epot
Et[2*nt+it] = Ekin + Epot
