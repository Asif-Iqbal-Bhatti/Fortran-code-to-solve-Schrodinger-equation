...
nplot = sum((2*l+1) for l in range(lmax+1))
...
for l in range(lmax+1):                                            # l-loop
   for m in range (-l,l+1):                                       # m-loop
      ...
      fxmin = xplot + 0.1*dplotx
      fxmax = xplot + 0.9*dplotx       # viewport
      fymin = yplot + 0.1*dploty
      fymax = yplot + 0.9*dploty
      title = f"l = {repr(l)},  m = {repr(m)}"
